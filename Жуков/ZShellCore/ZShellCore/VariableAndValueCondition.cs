...
namespace ZShellCore
{
	...
	public class VariableAndValueCondition : Condition
	{
		...
		public VariableAndValueCondition(Variable variable, ConditionComparer comparer, Value value) : base(comparer)
		{
			...
			{
				throw new ArgumentNullException("Типы переменной и значения не совпадают");
			}
		...
		public override bool compare()
		{
			...
			else
			{
				...
				if (comparable == null)
				{
					throw new ArgumentNullException("Значение переменной " + this.usedVariables[0].ToString() + " не поддерживает интерфейс IComparable");
				}
				result = this.comparer.compare(comparable, this.value.Data as IComparable);
			}
			return result;
		}
