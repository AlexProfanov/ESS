	...
	internal static class DialogFuncs
	{
		internal static void doDragBeginning(ListView listView)
		{
			if (listView.SelectedItems.Count > 0)
			{
				...
				int num = listView.DoDragDrop(listViewItem, 3);
			}
		}
		...
		internal static void doDragOver(ListView listView, DragEventArgs e)
		{
			...
			else
			{
				...
				e.Effect = ((e.KeyState & 4) != 4 || (e.AllowedEffect & 1) != 1) ? 2 : 1;
		...
		internal static bool needToSaveChanges(FormClosingMethod formClosingMethod)
		{
			...
			switch (formClosingMethod)
			{
				...
				case FormClosingMethod.EXIT:
					if (MessageBox.Show("Cохранить результаты?", "Внимание", 4, 32) == 6)
					{
						result = true;
					}
					break;
		...
		internal static void showDomainForSelectedVariable1(ComboBox comboBox, Variable variable)
		{
			comboBox.Items.Clear();
			if (variable.Domain != null)
			{
				...
				while (enumeratorForValues.MoveNext())
				{
					Value arg_38_0 = enumeratorForValues.Current;
					Value arg_3F_0 = enumeratorForValues.Current;
					bool flag = 0 == 0;
				}
				if (comboBox.Items.Count != 0)
				{
		...
		internal static void setComparerToSelect(ConditionComparer comparer, ComboBox comboBox)
		{
			...
			else if (comparer is ConditionNotEqualComparer)
			{
				comboBox.SelectedIndex = 1;
			}
			else if (comparer is ConditionAGreaterBComparer)
			{
